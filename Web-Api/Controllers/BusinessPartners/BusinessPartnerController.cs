...
namespace Web_Api.Controllers.BusinessPartners
{
    ...
    public class BusinessPartnerController : ControllerBase
    {
        ...
        public BusinessPartnerController(IBusinessPartnerService service, IMapper mapper, ILogger<BusinessPartnerController> logger)
        {
            _service = service;
            _mapper = mapper;
            _logger = logger;
        ...
        [HttpGet]
        public async Task<IEnumerable<BusinessPartnerDto>> GetWithPagination([FromQuery]int page = 0, [FromQuery]int size=10, [FromQuery] string modifiedAfter = null)
        {
        ...
        {
            _logger.LogDebug($"Get Customer By Key {key}");
            return _mapper.Map<BusinessPartnerDto>(await _service.GetByKeyAsync(key));
        }
        

        // POST: api/BusinessPartner
        [HttpPost]
        public async Task<BusinessPartnerDto> AddNew([FromBody]BusinessPartnerDto businessPartner, CancellationToken cancellationToken)
        {
            _logger.LogDebug($"Adding new customer {businessPartner.Name}");
            var result =await _service.AddBusinessPartnerAsync(_mapper.Map<BusinessPartner>(businessPartner), cancellationToken);
            return _mapper.Map<BusinessPartnerDto>(result);
        }

        // PUT: api/BusinessPartner
        [HttpPut("{cid}")]
        public async Task<BusinessPartnerDto> Update([FromRoute] string cid, [FromBody] BusinessPartnerDto businessPartner, CancellationToken cancellationToken)
        {
